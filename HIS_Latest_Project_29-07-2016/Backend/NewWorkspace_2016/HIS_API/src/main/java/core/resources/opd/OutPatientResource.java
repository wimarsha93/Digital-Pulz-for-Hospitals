...
/*	*//**
	 ...
	 */
	@GET
	@Path("/getPatientOfDoctorBySearchType/{docid}/{visit_type}/{searchType}/{searchValue}")
	@Produces(MediaType.APPLICATION_JSON)
	public String getPatientOfDoctorBySearchType(@PathParam("docid")int docid,
			@PathParam("visit_type")int visit_type,
			@PathParam("searchType")int searchType,@PathParam("searchValue")String searchValue) throws JSONException
	{
		...
		try{
	/*	OutPatient patient =  new OutPatient();
		patient = patientDBDriver.getPatientsPatinentByFeild(docid,feild);
		JSONSerializer jsonSerializer = new JSONSerializer();
	 ...
	 */
	@GET
	@Path("/getPatientsForDoctorByFeild/{userid}/{visitype}/{type}/{feild}")
	@Produces (MediaType.APPLICATION_JSON)
	public String getPatientsForDoctorByFeild(@PathParam("userid")int userID,@PathParam("visitype")int visitType,@PathParam("type")String type,@PathParam("feild")String feild) throws JSONException {
		
		System.out.println(userID + "" + visitType + "" +type +""+ feild );
		log.info("Entering the get patients for Doctor method");
		try 
		{
		   List<Visit> uList = patientDBDriver.getPatientsForDoctor(userID, visitType,type,feild);
